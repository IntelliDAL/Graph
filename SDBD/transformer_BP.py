...
runtime_id = 'BP-tansformer-{}'.format(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()))
...
X = np.load('./data/BP_HC_392_18A.npy')
...
#             loss += losses
    ...
    # Model.to('cpu')
    # train_data_batch = torch.from_numpy(X_train).float()
    # train_data_batch = Model.tsne_feature(train_data_batch)
    # test_data_batch = torch.from_numpy(X_test).float()
    # test_data_batch = Model.tsne_feature(test_data_batch)
    # for i in range(18):
    #     tmp_data = train_data_batch[:,i,:].cpu().detach().numpy()
    #     tmp_data = tsne.fit_transform(tmp_data)
    #     plot_embedding(tmp_data, Y_train, 'BP/train/kfold{}'.format(kfold_index), i)
    #     tmp_data = test_data_batch[:,i,:].cpu().detach().numpy()
    #     tmp_data = tsne.fit_transform(tmp_data)
    #     plot_embedding(tmp_data, Y_test, 'BP/test/kfold{}'.format(kfold_index), i)
