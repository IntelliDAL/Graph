...
runtime_id = 'disentangle-BP-{}'.format(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()))
...
logger.info('./data/BP_HC_392_18A')
logger.info('---------------------')
...
# torch.save(Model.state_dict(), './model_checkpoint/BD/BP-epochs{}-a{:.2}-b{:.2}-{:.4}-{}.pth'.format(epochs, args.a, args.b, A_loss+s_loss+d_loss, time.strftime('%Y-%m-%d-%H:%M:%S', time.localtime())))
torch.save(Model, './model_checkpoint/BD/BP-epochs{}-a{:.2}-b{:.2}-{:.4}-{}.pth'.format(epochs, args.a, args.b, A_loss+s_loss+d_loss, time.strftime('%Y-%m-%d-%H:%M:%S', time.localtime())))
logger.info('save in ./model_checkpoint/BD/BP-epochs{}-a{:.2}-b{:.2}-{:.4}-{}.pth'.format(epochs, args.a, args.b, A_loss+s_loss+d_loss, time.strftime('%Y-%m-%d-%H:%M:%S', time.localtime())))