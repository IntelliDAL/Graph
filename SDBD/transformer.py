...
from adaptive.models import MultiHeadSeqAttention
from utils import get_args
import logging

args = get_args()

runtime_id = 'tansformer-{}'.format(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()))
...
class EncoderDecoder(nn.Module):
    ...
    def __init__(self, encoder, src_embed, d_model1, d_model2):
        ...
        # self.BrainCNN = BrainCNN(d_model1, 8)
        # logger.info('use BrainCNN')

        # a = float(args.checkpoint[35:38])
        # b = float(args.checkpoint[40:43])
        a = 1.0
        b = 1.0
        ...
        self.pre.load_state_dict(torch.load(args.checkpoint, map_location='cuda:0'))
        # self.pre = torch.load(args.checkpoint)

        self.fold = nn.Sequential(
                            nn.Linear(200*8, d_model1),
                            nn.Dropout(0.5),
                            nn.ReLU()
                        )
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, A, mask):
        ...
        x, loss = self.sublayer[0](x, A, lambda x: self.self_attn1(x, x, x))
...
def make_model(N=6, d_model1=72, d_model2=200, d_ff=2048, h1=8, h2=10, dropout=0.5):
    ...
    attn1 = MultiHeadSeqAttention(d_model1, h1)
...
#     print(Y_test)
    ...
    # model
    Model = make_model(2, 32, 8, 128, 2, 2)
    Model.to(device)
#     for p in Model.parameters():
#         if p.requires_grad:
...
#             loss += losses
        ...
        if epoch % 5 == 0:
            ...
            # torch.save(Model, './model_checkpoint/ABIDE_6_4/ABIDECC200-epochs{}-{}.pth'.format(epoch, time.strftime('%Y-%m-%d-%H:%M:%S', time.localtime())))
        ...
        if epoch % 5 == 0:
            ...
            # A_bacth = torch.from_numpy(A_test).float().to(device)
    ...
    torch.save(Model, './model_checkpoint/ABIDE_6_4/ABIDECC200-epochs{}-{}-{}.pth'.format(epoch, kfold_index, time.strftime('%Y-%m-%d-%H:%M:%S', time.localtime())))
...
num = len(acc_list) // 5
for i in range(num):
    logging.info('{}-{}-{}-{}-{:.4}-{:.4}-{:.4}-{:.4}-{:.4}'.format(epoch_list[i], lr, batch_size, decay, acc_list[i], acc_list[i+num], acc_list[i+2*num], acc_list[i+3*num], acc_list[i+4*num]))
    if max_acc < (acc_list[i]+acc_list[i+num]+acc_list[i+2*num]+acc_list[i+3*num]+acc_list[i+4*num]) / 5:
        max_acc = (acc_list[i]+acc_list[i+num]+acc_list[i+2*num]+acc_list[i+3*num]+acc_list[i+4*num]) / 5
        max_precision = (precision_list[i]+precision_list[i+num]+precision_list[i+2*num]+precision_list[i+3*num]+precision_list[i+4*num]) / 5
        max_recall = (recall_list[i]+recall_list[i+num]+recall_list[i+2*num]+recall_list[i+3*num]+recall_list[i+4*num]) / 5
        max_f1 = (f1_list[i]+f1_list[i+num]+f1_list[i+2*num]+f1_list[i+3*num]+f1_list[i+4*num]) / 5
        max_auc = (auc_list[i]+auc_list[i+num]+auc_list[i+2*num]+auc_list[i+3*num]+auc_list[i+4*num]) / 5
        max_epoch = epoch_list[i]
    
with open('./result/{}/resultMax.txt'.format(args.save), "a") as file:
        file.write('{}-{}-{}-{}-{:.4}-{:.4}-{:.4}-{:.4}-{:.4}\n'.format(max_epoch, lr, batch_size, decay, max_acc, max_precision, max_recall, max_f1, max_auc))
