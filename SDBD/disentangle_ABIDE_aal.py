...
runtime_id = 'disentangle_ABIDE_aal-{}'.format(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()))
...
torch.save(Model.state_dict(), './model_checkpoint/ABIDE_aal_5/epochs{}-a{:.2}-b{:.2}-{:.4}-{}.pth'.format(epochs, args.a, args.b, A_loss+s_loss+d_loss, time.strftime('%Y-%m-%d-%H:%M:%S', time.localtime())))
...
logger.info('save in ./model_checkpoint/ABIDE_aal_5/epochs{}-a{:.2}-b{:.2}-{:.4}-{}.pth'.format(epochs, args.a, args.b, A_loss+s_loss+d_loss, time.strftime('%Y-%m-%d-%H:%M:%S', time.localtime())))