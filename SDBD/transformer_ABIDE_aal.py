...
from model_ABIDE_aal import my_model
from utils import get_args
import logging

args = get_args()

runtime_id = 'tansformer-ABIDE-aal-{}'.format(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()))
...
class EncoderDecoder(nn.Module):
    ...
    def __init__(self, encoder, src_embed, d_model1, d_model2):
        ...
        self.linear = nn.Sequential(
                            ...
                            # nn.Dropout(0.5),
                            # nn.ReLU(),
                            nn.Linear(d_model1*d_model2, 64),
                            nn.Dropout(0.5),
                            nn.ReLU(),
                            nn.Linear(64, 2)
                        )
...
X = np.load('./data/ABIDE_aal_8A.npy')
...
#     print(Y_test)
    ...
    # model
    Model = make_model(1, 128, 8, 128, 2, 2)
    Model.to(device)
#     for p in Model.parameters():
#         if p.requires_grad:
...
for i in range(num):
    ...
    logger.info('\n')
    with open('./result/{}/result.txt'.format(args.save), "a") as file:
