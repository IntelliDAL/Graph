...
from frameworks.utils import create_activation, NormLayer, create_norm
...
class GIN(nn.Module):
    ...
    def forward(self, g, inputs, strufea=None, return_hidden=False):
        h = inputs
        hidden_list = []
        for l in range(self.num_layers):
            if (l == 1) and (strufea is not None):
                h = h + strufea
