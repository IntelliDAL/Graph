...
def load_dataset(dataset_name):
    ...
    if dataset_name.startswith("ogbn"):
        dataset = GRAPH_DICT[dataset_name](dataset_name, raw_dir='./../GraphData')
    else:
        dataset = GRAPH_DICT[dataset_name](raw_dir='./../GraphData')
...
def load_inductive_dataset(dataset_name):
    if dataset_name == "ppi":
        ...
        # create the dataset
        train_dataset = PPIDataset(mode='train', raw_dir='./../GraphData')
        valid_dataset = PPIDataset(mode='valid', raw_dir='./../GraphData')
        test_dataset = PPIDataset(mode='test', raw_dir='./../GraphData')
    ...
    if "attr" not in graph.ndata:
        if "node_labels" in graph.ndata and not deg4feat:
            ...
            for g, l in dataset:
                ...
                g.ndata["attr"] = feat # 原有
                # g.ndata["attr"] = feat[:, :271] # 后加 # 177

            # feature_dim = 271   # feature_dim
