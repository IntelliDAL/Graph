...
print(device)
# Set_seed.
# We tried random seeding several times and found that the final effect was largely unaffected
setup_seed(10)
pre_epo = 10
...
for ind in range(5):
    ...
    for trainval_index, test_index in kf.split(X, Y):
        ...
        model.to(device)
        optimizer1 = optim.SGD(model.parameters(), lr=lr/5, weight_decay=decay, momentum=0.9, nesterov=True) #for pre-train
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch <= pre_epo:
                ...
                print(f"epoc:{epoch} semi loss: {loss} sparse rate :{(model.sigcnn.get_w_data() != 0).int().sum() / (116*116)}")
