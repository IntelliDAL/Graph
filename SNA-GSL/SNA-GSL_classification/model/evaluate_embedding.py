...
def svc_classify(x, y, class_num, search):
    kf = StratifiedKFold(n_splits=10, shuffle=True, random_state=None)
    accuracies = []
    accuracies_val = []
    aucs = []
    ...
    for train_index, test_index in kf.split(x, y):
        ...
        if class_num > 2:
            y_onehot = torch.zeros(len(y_pred), class_num, dtype=torch.int64)
            y_ = torch.tensor(y_pred).unsqueeze(-1)
            y_onehot = y_onehot.scatter(1, y_, 1)
            aucs.append(roc_auc_score(y_test, y_onehot, multi_class='ovo'))
        else:
        ...
        classifier.fit(x_train, y_train)
        y_pred = classifier.predict(x_test)
        accuracies_val.append(accuracy_score(y_test, y_pred))
        if class_num > 2:
            y_onehot = torch.zeros(len(y_pred), class_num, dtype=torch.int64)
            y_ = torch.tensor(y_pred).unsqueeze(-1)
            y_onehot = y_onehot.scatter(1, y_, 1)
            aucs_val.append(roc_auc_score(y_test, y_onehot, multi_class='ovo'))
        else:
            aucs_val.append(roc_auc_score(y_test, y_pred))
        f1_val.append(f1_score(y_test, y_pred, average='micro'))
    # print("查看真实值Y")
    # print(y_test)
    # print("查看预测值Y")
    # print(y_pred)
    # _acc_val, _acc,_auc_val, _auc,_f1,_f1_val
    return np.mean(accuracies_val), np.mean(accuracies), np.mean(aucs_val), np.mean(aucs), np.mean(f1), np.mean(f1_val)
...
def evaluate_embedding(embeddings, labels, class_num, search=False):
    labels = preprocessing.LabelEncoder().fit_transform(labels)
    x, y = np.array(embeddings), np.array(labels)
    ...
    _acc_val, _acc, _auc_val, _auc, _f1, _f1_val = svc_classify(x, y, class_num, search)
    if _acc_val > acc_val:
        acc_val = _acc_val
        acc = _acc
    ...
    print("acc_val:", acc_val, "acc:", acc)

    return _acc_val, _acc, _auc_val, _auc, _f1, _f1_val
