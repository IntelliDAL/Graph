...
class E2EBlock(nn.Module):
    def __init__(self, args, in_channel, out_channel):
        ...
        if self.args.n_max_nodes > args.pooling_res:
...
class SimCNN(torch.nn.Module):
    def __init__(self, args):
        ...
        self.E2N = torch.nn.Conv2d(in_channels=args.conv_channels_1, out_channels=args.conv_channels_2,
                                   kernel_size=(1, self.d))
        ...
        self.fc_4 = torch.nn.Linear(args.conv_channels_3 // 8, self.args.num_classes)
    ...
    def forward(self, sim_mat, mask_ij):
        ...
        # out = torch.einsum('bhij,bij->bhij', F.relu(self.e2econv1(x), inplace=True), mask_ij)
        # out = torch.einsum('bhij,bij->bhij', F.relu(self.e2econv2(out), inplace=True), mask_ij)
        #
        out = F.leaky_relu(self.E2N(out), negative_slope=self.args.conv_l_relu_slope, inplace=True)
        out = F.dropout(F.leaky_relu(self.N2G(out), negative_slope=self.args.conv_l_relu_slope, inplace=True),
                        p=self.args.conv_dropout, training=self.training).squeeze()

        # out=out.reshape(-1,3,out.shape[-1])
        # out=torch.sum(out,dim=1)
        out = F.dropout(F.leaky_relu(self.fc_1(out), negative_slope=self.args.conv_l_relu_slope, inplace=True),
                        p=self.args.conv_dropout, training=self.training)
        out = F.dropout(F.leaky_relu(self.fc_2(out), negative_slope=self.args.conv_l_relu_slope, inplace=True),
                        p=self.args.conv_dropout, training=self.training)
        out = F.dropout(F.leaky_relu(self.fc_3(out), negative_slope=self.args.conv_l_relu_slope, inplace=True),
                        p=self.args.conv_dropout, training=self.training)
