...
from torch_geometric.utils import degree, to_dense_batch, to_dense_adj, dense_to_sparse
...
class TUDataset_aug(InMemoryDataset):
    ...
    def transform(self, data1, data2):
        new_data = dict()
        b0 = to_dense_batch(data1.x, batch=data1.batch, max_num_nodes=self.n_max_nodes)
        g0 = {
            'adj': to_dense_adj(data1.edge_index, batch=data1.batch, max_num_nodes=self.n_max_nodes).to(
                self.args.device),
            'x': b0[0].to(self.args.device),
            'mask': b0[1].to(self.args.device),
            'batch': data1.batch.to(self.args.device),
            'dist': None if not self.args.use_dist else self._get_normed_dis_tensor(data1['i']).to(self.args.device),
            'sparse_edge_index': data1.edge_index
        }
        b1 = to_dense_batch(data2.x, batch=data2.batch, max_num_nodes=self.n_max_nodes)
        g1 = {
            'adj': to_dense_adj(data2.edge_index, batch=data2.batch, max_num_nodes=self.n_max_nodes).to(
                self.args.device),
            'x': b1[0].to(self.args.device),
            'mask': b1[1].to(self.args.device),
            'batch': data2.batch.to(self.args.device),
            'dist': None if not self.args.use_dist else self._get_normed_dis_tensor(data2['i']).to(self.args.device),
            'sparse_edge_index': data2.edge_index
        }
        new_data['g0'] = g0
        new_data['g1'] = g1
        return new_data
